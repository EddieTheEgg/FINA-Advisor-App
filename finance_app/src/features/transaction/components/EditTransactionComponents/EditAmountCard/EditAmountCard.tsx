...
import { styles } from './EditAmountCard.styles';

export const EditAmountCard = () => {

    const {amountDraft, setAmountDraft, sourceAccountDraft, validateAmount, transactionTypeDraft, amountError} = useEditTransactionStore();
    const [inputValue, setInputValue] = useState<string>(amountDraft > 0 ? amountDraft.toFixed(2) : '');

    useEffect(() => {
        validateAmount();
    }, [amountDraft, validateAmount, sourceAccountDraft]);
